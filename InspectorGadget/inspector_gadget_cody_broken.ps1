...
$script:unique_wrong_count = 0
$displayunique = ( $wmiinfo | Select SocketDesignation -unique | Measure-Object ).count
...
Function writeSystemInfo
{
	...
	if ((!$OS) -or ($OS -like "*2003*")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$OS</td>";$script:things_wrong++;$script:os_wrong+="<a href='#$server'>$server</a>, ";$script:os_wrong_count++;$script:unique_wrong_count++} 
	...
	elseif (($activation -notlike "Licence status: Licensed") -and ($activation -notlike "Not Applicable")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$activation</td>";$script:things_wrong++;$script:activation_wrong+="<a href='#$server'>$server</a>, ";$script:activation_wrong_count++;$script:unique_wrong_count++}
	...
	if ((!$proxycfg) -or ($proxycfg -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$proxycfg</td>";$script:things_wrong++;$script:proxy_wrong+="<a href='#$server'>$server</a>, ";$script:proxy_wrong_count++;$script:unique_wrong_count++} 
...
Function writeDiskInfo
{
	...
	if (($devId -match "C") -and ($volName -notmatch "$server")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$volName</td>";$script:things_wrong++;$script:diskname_wrong+="<a href='#$server'>$server</a>, ";$script:diskname_wrong_count++;$script:unique_wrong_count++}
	...
	elseif ($freePercent -le $critical) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$freePercent</td>";$script:things_wrong++;$script:diskcrit_wrong+="<a href='#$server'>$server</a>, ";$script:diskcrit_wrong_count++;$script:unique_wrong_count++}
...
Function writeNetworkInfo
{
	...
	Add-Content $InspectorGadgetFile "<tr>"
	if (!$IP) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$IP</td>";$script:things_wrong++;$script:ip_wrong+="<a href='#$server'>$server</a>, ";$script:ip_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$IP</td>"}
	if ((!$DNS) -or ($DNS -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$DNS</td>";;$script:things_wrong++;$script:dns_wrong+="<a href='#$server'>$server</a>, ";$script:dns_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$DNS</td>"}
	if ((!$MTU) -or ($MTU -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$MTU</td>";$script:things_wrong++;$script:mtu_wrong+="<a href='#$server'>$server</a>, ";$script:mtu_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$MTU</td>"}
	if ((!$NetBIOS) -or ($NetBIOS -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$NetBIOS</td>";$script:things_wrong++;$script:netbios_wrong+="<a href='#$server'>$server</a>, ";$script:netbios_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$NetBIOS</td>"}
	if ((!$ipv6) -or ($ipv6 -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$ipv6</td>";$script:things_wrong++;$script:ipv6_wrong+="<a href='#$server'>$server</a>, ";$script:ipv6_wrong_count++;$script:unique_wrong_count++} 
...
Function writeMiscInfo
{
	...
	if ((!$httperr) -or ($httperr -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$httperr</td>";$script:things_wrong++;$script:httperr_wrong+="<a href='#$server'>$server</a>, ";$script:httperr_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$httperr</td>"}
	if ((!$remotemgmt) -or ($remotemgmt -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$remotemgmt</td>";$script:things_wrong++;$script:rmtmgmt_wrong+="<a href='#$server'>$server</a>, ";$script:rmtmgmt_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$remotemgmt</td>"}
	if ((!$UAC) -or ($UAC -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$UAC</td>";$script:things_wrong++;$script:uac_wrong+="<a href='#$server'>$server</a>, ";$script:uac_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$UAC</td>"}
	if ((!$XPBY) -or ($XPBY -notmatch "$server")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$XPBY</td>";$script:things_wrong++;$script:xpby_wrong+="<a href='#$server'>$server</a>, ";$script:xpby_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$XPBY</td>"}
	if ($server -match "REBELLS|REBELWEBTEST02|REBELWS1|QTWSPARE2|REBELBLAST1|BEARDRWEB1|BEARDRDOC1|BEARSRCHBAT1|BEARSRCHBAT2|LUIGIBOASYNC1|LUIGIBOBATCH1|LUIGIBOBLAST|LUIGIBOWS1|LUIGIMAILBOSM1|LUIGIMX1|LUIGIMXWS1|LUIGIDPIBATCH1|LUIGIDPIBLAST1|LUIGIDPIFEED1|LUIGIDPIMAIL1|LUIGIDPISVC1|LUIGIDPISVC2|LUIGIMAP1|LUIGIMAP2|LUIGIMAP3|LUIGIMAP4|LUIGIBLAST|LUIGISPARE1|DRAGONBLAST1|AMSTELBLAST1|CBASYNC204|CBASYNC221|CBBLAST100|CBLS|DWTEST1|ORKINWEB2|REBELDW1|REBELDW2|REBELDW3|BOBATCH100|BOBATCH101|BOBLAST100|MAILBOSM1|MAILBOSM2|REBELBOASYNC1|REBELBOASYNC2|REBELBOWS1|REBELBOWS2|REBELMX100|REBELMX101|REBELMX102|REBELMXWS100|REBELMXWS101|REBELNXS1|REBELNXS2|BEARDPIBATCH1|BEARDPIBATCH2|BEARDPISVC1|BEARDPISVC2|BEARDPIWEB1|BEARDPIWEB2|DPIBATCH1Q|DPIBATCH2Q|DPIBLAST1Q|DPIFEED1Q|DPIFEED2Q|DPIMAIL1Q|DPIMAIL2Q|DPISVC1Q|DPISVC2Q|DPISVC3Q|DPISVC4Q|REBELDPIBATCH1|REBELDPIBATCH2|REBELDPISVC1|REBELDPISVC2|REBELDPIWEB1|REBELDPIWEB2|QTMMAP1|QTMMAP10|QTMMAP2|QTMMAP3|QTMMAP4|QTMMAP5|QTMMAP6|QTMMAP7|QTMMAP8|QTMMAP9|DEVDFS1|DEVVMCTR|GHQBLAST|GHQBLAST01|GHQDEPLOYTEST1|GHQDEPLOYTEST2|GHQDEPLOYTEST3|GHQFLASH|GHQHADOOPUTIL|GHQJSTEST|GHQLS|GHQTEST0|GHQTEST1|GHQTEST10|GHQTEST11|GHQTEST12|GHQTEST13|GHQTEST14|GHQTEST15|GHQTEST16|GHQTEST17|GHQTEST18|GHQTEST19|GHQTEST2|GHQTEST20|GHQTEST21|GHQTEST3|GHQTEST4|GHQTEST5|GHQTEST6|GHQTEST7|GHQTEST8|GHQTEST9|PIREPORT1|SPARETESTER1") {Add-Content $InspectorGadgetFile "<td>F5fwd: $F5fwd<br />ISAPI Filters:$ISAPI</td>"}
	elseif ((!$F5fwd) -or ($F5fwd -notmatch "F5ForwardedFor32")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$F5fwd</td>";$script:things_wrong++;$script:F5fwd_wrong+="<a href='#$server'>$server</a>, ";$script:F5fwd_wrong_count++;$script:unique_wrong_count++} 
...
Function writeTimeDirInfo
{
	...
	if (!$InstallDate) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$InstallDate</td>";$script:things_wrong++;$script:install_wrong+="<a href='#$server'>$server</a>, ";$script:install_wrong_count++;$script:unique_wrong_count++} 
	elseif ($InstallDate -lt (get-Date).addmonths(-36)) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$InstallDate</td>";$script:things_wrong++;$script:installdate_wrong+="<a href='#$server'>$server</a>, ";$script:installdate_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td>$InstallDate</td>"}
	if (!$LastBootUpTime) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$LastBootUpTime</td>";$script:things_wrong++;$script:lastboot_wrong+="<a href='#$server'>$server</a>, ";$script:lastboot_wrong_count++;$script:unique_wrong_count++} 
	...
	if ((!$TimeZone) -or ($TimeZone -match "Wrong")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$TimeZone</td>";$script:things_wrong++;$script:timezone_wrong+="<a href='#$server'>$server</a>, ";$script:timezone_wrong_count++;$script:unique_wrong_count++}	
	else {Add-Content $InspectorGadgetFile "<td>$TimeZone</td>"}
	if (($Batch -match "empty|exist") -or ($IIS -match "empty|exist")) {Add-Content $InspectorGadgetFile "<tdbgcolor='#FF0000' align=center>$Batch<br/>$IIS</td>";$script:things_wrong++;$script:dirs_wrong+="<a href='#$server'>$server</a>, ";$script:dirs_wrong_count++;$script:unique_wrong_count++}	
	else {Add-Content $InspectorGadgetFile "<td>$Batch<br/>$IIS</td>"}
	if (($hostsfile -notlike "127.0.0.1       localhost`n::1             localhost") -and ($hostsfile -notlike "127.0.0.1       localhost")){Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center>$hostsfile</td>";$script:things_wrong++;$script:hostsfile_wrong+="<a href='#$server'>$server</a>, ";$script:hostsfile_wrong_count++;$script:unique_wrong_count++}
...
Function writeDellInfo
{
	...
	Add-Content $InspectorGadgetFile "<tr>"
	if ((!$BIOS) -or ($BIOS -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$BIOS_Settings</td>";$script:things_wrong++;$script:bios_wrong+="<a href='#$server'>$server</a>, ";$script:bios_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td colspan='2'>$BIOS</td>"}
	if ((!$Firmeware) -or ($Firmeware -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$Firmeware</td>";$script:things_wrong++;$script:firmware_wrong+="<a href='#$server'>$server</a>, ";$script:firmware_wrong_count++;$script:unique_wrong_count++} 
	else {Add-Content $InspectorGadgetFile "<td colspan='2' valign='top'>$Firmeware</td>"}
	if ((!$iDracInfo) -or ($iDracInfo -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$iDracInfo</td>";$script:things_wrong++;$script:iDracInfo_wrong+="<a href='#$server'>$server</a>, ";$script:iDracInfo_wrong_count++;$script:unique_wrong_count++} 
...
Function writeTaskInfo
{
	...
	if ($its_wrong -eq "1") {Add-Content $InspectorGadgetFile "<td colspan='6' bgcolor='#FF0000' align=center>$schedtasks</td>";$script:things_wrong++;$script:tasks_wrong+="<a href='#$server'>$server</a>, ";$script:tasks_wrong_count++;$script:unique_wrong_count++}
...
if ($email -eq "me")
{sendEmail "Inspector Gadget <InspectorGadget@careerbuilder.com>" "SiteServers <cody.rucks@careerbuilder.com>" "Inspector Gadget - Windows: $date, $server_count servers, $things_wrong things wrong in servers" $InspectorGadgetFileName}
