...
Function writeMiscInfo
{
	...
	if ((!$F5fwd) -or ($F5fwd -notmatch "F5ForwardedFor32")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$F5fwd</td>";$script:things_wrong++;$script:F5fwd_wrong+="<a href='#$server'>$server</a>, "} 
...
Function writeTimeDirInfo
{
	...
	if (($Batch -match "empty|exist") -or ($IIS -match "empty|exist")) {Add-Content $InspectorGadgetFile "<tdbgcolor='#FF0000' align=center>$Batch<br/>$IIS</td>";$script:things_wrong++;$script:dirs_wrong+="<a href='#$server'>$server</a>, "}	
	else {Add-Content $InspectorGadgetFile "<td>$Batch<br/>$IIS</td>"}
	if (($hostsfile -notlike "127.0.0.1       localhost`n::1             localhost") -and ($hostsfile -notlike "127.0.0.1       localhost")){Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center>$hostsfile</td>";$script:things_wrong++;$script:hostsfile_wrong+="<a href='#$server'>$server</a>, "}
...
Function writeDellInfo
{
	...
	Add-Content $InspectorGadgetFile "<tr>"
	if ((!$BIOS) -or ($BIOS -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$BIOS_Settings</td>";$script:things_wrong++;$script:bios_wrong+="<a href='#$server'>$server</a>, "} 
	else {Add-Content $InspectorGadgetFile "<td colspan='2'>$BIOS</td>"}
	if ((!$Firmeware) -or ($Firmeware -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$Firmeware</td>";$script:things_wrong++;$script:firmware_wrong+="<a href='#$server'>$server</a>, "} 
	else {Add-Content $InspectorGadgetFile "<td colspan='2' valign='top'>$Firmeware</td>"}
	if ((!$iDracInfo) -or ($iDracInfo -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center valign='top'>$iDracInfo</td>";$script:things_wrong++;$script:iDracInfo_wrong+="<a href='#$server'>$server</a>, "} 
...
Function writeTaskInfo
{
	...
	if ($its_wrong -eq "1") {Add-Content $InspectorGadgetFile "<td colspan='6' bgcolor='#FF0000' align=center>$schedtasks</td>";$script:things_wrong++;$script:tasks_wrong+="<a href='#$server'>$server</a>, "}
...
($timezone_wrong) -or ($dirs_wrong) -or ($tasks_wrong) -or ($servers_offline) -or ($bios_wrong) -or ($firmware_wrong) -or ($idracinfo_wrong) -or ($hostsfile_wrong) -or ($F5fwd_wrong))
{
	...
	if($F5fwd_wrong){$prereport += "<b>Servers with incorrect F5fwd module:</b><br />$F5fwd_wrong <br /><br />"}
	...
	if($hostsfile_wrong){$prereport += "<b>Servers that have a modified hosts file:</b><br />$hostsfile_wrong <br /><br />"}
	if($bios_wrong){$prereport += "<b>Servers unable to get BIOS settings from:</b><br />$bios_wrong <br /><br />"}
	if($firmware_wrong){$prereport += "<b>Servers unable to get Firmware versions from:</b><br />$firmware_wrong <br /><br />"}
	if($idracinfo_wrong){$prereport += "<b>Servers unable to get iDRAC information from:</b><br />$idracinfo_wrong <br /><br />"}
