...
<#$asyncFunctions = 
{
	...
	Function writeMiscInfo
	{
		...
		if ((!$XPBY) -or ($XPBY -notmatch "$server")) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$XPBY</td>";$async_server.things_wrong++;$async_server.xpby_wrong+="<a href='#$server'>$server</a>, ";$async_server.xpby_wrong_count++} 
	...
	Function writeTimeDirInfo
	{
		...
		if (($Batch -match "empty|exist") -or ($IIS -match "empty|exist")) {Add-Content $InspectorGadgetFile "<tdbgcolor='#FF0000' align=center>$Batch<br/>$IIS</td>";$async_server.things_wrong++;$async_server.dirs_wrong+="<a href='#$server'>$server</a>, ";$async_server.dirs_wrong_count++}	
		else {Add-Content $InspectorGadgetFile "<td>$Batch<br/>$IIS</td>"}
		if (($hostsfile -notlike "127.0.0.1       localhost`n::1             localhost") -and ($hostsfile -notlike "127.0.0.1       localhost")){Add-Content $InspectorGadgetFile "<td colspan='2' bgcolor='#FF0000' align=center>$hostsfile</td>";$async_server.things_wrong++;$async_server.hostsfile_wrong+="<a href='#$server'>$server</a>, ";$async_server.hostsfile_wrong_count++}
