	...
	if ($async_server.name -notmatch  "QTWLS|BEARLS|DRAGONLS|AMSTELLS|QTMSTATSLS")
	{
		...
		$jobs += Start-Job -ScriptBlock {
			...
			Function writeTimeDirInfo
			{
				...
				elseif ($InstallDate -lt (get-Date).addmonths(-36)) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>$InstallDate</td>";$async_server.installdate_wrong+="<a href='#$server'>$server</a>, ";$async_server.installdate_wrong_count++}  
