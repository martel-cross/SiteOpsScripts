...
$script:ipv6_wrong,$script:dynports_wrong,$script:httperr_wrong,$script:rmtmgmt_wrong,$script:uac_wrong,$script:xpby_wrong,$script:isapi_wrong,$script:installdate_wrong,$script:tcpwait_wrong, `
...
Function writeNetworkTableHeader
{
	...
	<td align='center'><strong>TCP/IP Wait Value</strong></td>"
}
...
{
	param($InspectorGadgetFile,$server,$IP,$DBIP,$thisCaption,$tcpwait)	
	Add-Content $InspectorGadgetFile "<tr>"
	...
	#Add-Content $InspectorGadgetFile "</tr>"
	if (!$tcpwait) {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>'No Value set!'</td>";$script:things_wrong++;$script:tcpwait_wrong+="<a href='#$server'>$server</a>, "} 
	elseif ($tcpwait -eq "120") {Add-Content $InspectorGadgetFile "<td bgcolor='#FF0000' align=center>Value not correct: $tcpwait</td>";$script:things_wrong++;$script:tcpwait_wrong+="<a href='#$server'>$server</a>, "} 
	#Add-Content $InspectorGadgetFile "<td>$tcpwait</td>"
}
...
foreach ($server in $serverlist)
{
	if ($server -notmatch  "QTWLS|BEARLS|DRAGONLS|AMSTELLS|QTMSTATSLS")
	{
		...
		$tcpwait = Get-Reg -Hive LocalMachine -Key "SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" -Value "TcpTimedWaitDelay" -RemoteComputer $server
		...
		writeNetworkInfo $InspectorGadgetFile $server $IP $DBIP $thisCaption $tcpwait
		Write-Host "$server $tcpwait done being checked"
		$thisCaption = ""
	}	
}
...
if (($tcpwait_wrong) -or ($servers_offline))
{
	...
	if($tcpwait_wrong){$prereport += "<b>TCP/IP Wait Value:</b><br />$tcpwait_wrong <br /><br />"}
}
...
if ($email -eq "me")
{sendEmail "Inspector Gadget <InspectorGadget@careerbuilder.com>" "SiteServers <cody.rucks@careerbuilder.com>" "Inspector Gadget - TCP/IP Wait Report: $date, $server_count servers, $things_wrong things wrong" $InspectorGadgetFileName}
