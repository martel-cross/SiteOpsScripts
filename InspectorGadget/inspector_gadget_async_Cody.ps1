...
if ($list -ne "custom")
{
    ...
    $ouArray += $([ADSI]"LDAP://OU=STRATUS,OU=DEVTEST,OU=Servers,DC=ATL,DC=CAREERBUILDER,DC=COM").path
...
$script:lastboot_wrong,$script:timezone_wrong,$script:dirs_wrong,$script:tasks_wrong,$script:bios_wrong,$script:firmware_wrong,$script:idracinfo_wrong,$script:hostsfile_wrong,$script:mem_wrong,$script:F5fwd_wrong = ""
...
$script:lastboot_wrong_count,$script:timezone_wrong_count,$script:dirs_wrong_count,$script:tasks_wrong_count,$script:bios_wrong_count,$script:firmware_wrong_count,$script:idracinfo_wrong_count,$script:hostsfile_wrong_count,$script:mem_wrong_count,$script:F5fwd_wrong_count = 0
	...
	if ($async_server.name -notmatch  "QTWLS|BEARLS|DRAGONLS|AMSTELLS|QTMSTATSLS")
	{
		...
		$jobs += Start-Job -ScriptBlock {
			...
			Function writeTimeDirInfo
			{
				...
				elseif ($InstallDate -lt (get-Date).addmonths(-36)) {Add-Content $InspectorGadgetFile "<td bgcolor='#FFFF00' align=center>$InstallDate</td>";$async_server.installdate_wrong+="<a href='#$server'>$server</a>, ";$async_server.installdate_wrong_count++} 
			...
			Function writeTaskInfo
			{
				...
				$its_wrong = ""
				Add-Content $InspectorGadgetFile "<tr>"
				if ((!$schedtasks) -and ($server -notmatch $task_exclude)) {$its_wrong = "1"} 
				elseif ((($schedtasks -match "Cycle") -and ($schedtasks -match "LastTaskResult=0")) -and (($schedtasks -notmatch "True") -and ($server -notmatch $task_exclude) -or ($server -notmatch "BAT"))) {Add-Content $InspectorGadgetFile "<td colspan='6' align=center>$schedtasks</td>"}
				if (($server -match "BAT") -and ($schedtasks -match "Name=Cycle; Enabled=True;" )) {$its_wrong ="1"}
				elseif ((($server -match "BAT") -and (($schedtasks -match "GrimReaper") -or ($schedtasks -match "IIS_Backup"))) -and (($schedtasks -eq "") -or ($schedtasks -notmatch "LastTaskResult=0"))) {$its_wrong = "1"}
				elseif ((($server -match "BAT") -and (($schedtasks -match "GrimReaper") -or ($schedtasks -match "IIS_Backup"))) -and (($schedtasks -match "LastTaskResult=0"))) {Add-Content $InspectorGadgetFile "<td colspan='6' align=center>$schedtasks</td>"}
				if ((($server -notmatch "BAT") -or ($server -notmatch $task_exclude)) -and ($schedtasks -notmatch "True")) {$its_wrong = "1"}
				if ((($server -notmatch "BAT") -or ($server -notmatch $task_exclude)) -and ($schedtasks -notmatch "Cycle") -or ($schedtasks -notmatch "Grimreaper") -or ($schedtasks -notmatch "IIS_Backup") -or ($schedtasks -eq "") -or ($schedtasks -notmatch "LastTaskResult=0") -or ($schedtasks -notmatch "GrimReaper2") -and (($server -notmatch $task_exclude) -or ($server -notmatch "GHQ") -or ($server -notmatch "bat"))) {$its_wrong = "1"}
				elseif ($its_wrong -eq "1") {Add-Content $InspectorGadgetFile "<td colspan='6' bgcolor='#FF0000' align=center>$schedtasks</td>";$async_server.things_wrong++;$async_server.tasks_wrong+="<a href='#$server'>$server</a>, ";$async_server.tasks_wrong_count++}
				else {Add-Content $InspectorGadgetFile "<td colspan='6'>$schedtasks</td>"}
				Add-Content $InspectorGadgetFile "</tr>"
			}
			
			Function writeMemTableHeader
			{
				param($InspectorGadgetFile)
				Add-Content $InspectorGadgetFile "<tr bgcolor=#CCCCCC>
				<td colspan='6' align='center'><strong>Memory Info</strong></td>
				</tr>"
			}
	
			Function writeMemInfo
			{
				param($InspectorGadgetFile,$mem)
				$mem_wrong = ""
				Add-Content $InspectorGadgetFile "<tr>"
				if ((!$MEM) -or ($MEM -match "Wrong")) {Add-Content $InspectorGadgetFile "<td colspan='6' valign='top'>Open Manage not Installed</td>"}
#				if ($MEM -contains "Total Installed Capacity : 4096 MB") {$mem_wrong = "1"}
#				foreach ($str in $MEM)
#				{
#					if ($str.contains("Total Installed Capacity"))
#					{
#						if((!$str.contains("16384 MB") -or ($str.contains("24576 MB"))))
#						{
#							$mem_wrong = "1"
#						}
#					}
#				}			
				#if ((!$MEM.Contains("Installed Capacity : 16384 MB")) -or (!$MEM.Contains("Installed Capacity : 24576 MB"))) {$mem_wrong = "1"}
				#if (($mem_wrong -eq "1") -and ($model -notmatch "VMware")) {Add-Content $InspectorGadgetFile "<td colspan='6' bgcolor='#FF0000' align=center valign='top'>$MEM</td>";$async_server.things_wrong++;$async_server.mem_wrong+="<a href='#server'>$server</a>, ";$async_server.mem_wrong_count++}
				else {Add-Content $InspectorGadgetFile "<td colspan='6' valign='top'>$MEM</td>"}
				Add-Content $InspectorGadgetFile "</tr>"
			}
			...
			$TotalRAM = [math]::round( ( $hw1.TotalPhysicalMemory / 1024 / 1024 / 1024 ) );$totalmemory = $async_server.totalmemory+=$totalram
			...
			if ($hw1.model -match "PowerEdge")
			{
				...
				writeDellInfo $async_tempFile $BIOS $Firmware $iDracInfo $MEM
			...
			writeMemTableHeader $async_tempFile
			
			$MEM = invoke-command -computername $async_server.name -scriptblock {omreport chassis memory} | Foreach-Object {$_ -replace "(?m)^", "<br />"}
			
			writeMemInfo $async_tempFile $MEM
...
foreach ($job in $jobs)
{
    ...
    $script:totalmemory += $jobResults.totalmemory
	...
	$script:mem_wrong += $jobResults.mem_wrong
	$script:mem_wrong_count += $jobResults.mem_wrong_count
...
if($totalmemory){$prereport += "<b>Total memory of all servers:</b><br />$totalmemory<br /><br />"}
if($ModelCount)
{
...
($timezone_wrong) -or ($dirs_wrong) -or ($tasks_wrong) -or ($servers_offline) -or ($bios_wrong) -or ($firmware_wrong) -or ($idracinfo_wrong) -or ($hostsfile_wrong) -or ($F5fwd_wrong) -or ($mem_wrong))
{
	...
	if($installdate_wrong){$prereport += "<b><font color='yellow'>$installdate_wrong_count </font> Servers with Install Date older than 3 years:</b><br />$installdate_wrong <br /><br />"}
	...
	if($mem_wrong){$prereport += "<b><font color='red'>$mem_wrong_count </font> Servers unable to get Memory settings from:</b><br />$mem_wrong <br /><br />"}
}
...
else {sendEmail "Inspector Gadget <InspectorGadget@careerbuilder.com>" "SiteBackend <sitebackend@careerbuilder.com>" "Inspector Gadget: $date, $server_count servers, $things_wrong things wrong" $InspectorGadgetFileName}