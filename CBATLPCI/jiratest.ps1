...
$issues | foreach{
if ($_.fields.Summary -like "*new hire*"){ $ticket = $_.fields.description.tostring() ; add-content test.txt $ticket ; $ticketsubstr = get-content test.txt | select-string "Manager" ; 
$ticketsubstr | foreach{
$_.tostring().trim("Manager").trim()
}
